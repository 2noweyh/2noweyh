# Hi there 👋 I'm Hyewon!

🌟 AI researcher working on **Graph-based & LLM-driven systems**, with a particular focus on **Retrieval-Augmented Generation (RAG)**.
Currently a Post-Master's Researcher at **Korea Atomic Energy Research Institute (KAERI)**.

🔗 **[Personal Homepage](https://2noweyh.github.io)** · **[Google Scholar](https://scholar.google.com/citations?user=1arNStIAAAAJ)** · 📫 [noweyh927@gmail.com](mailto:noweyh927@gmail.com)

---

## 🚀 About Me

- 🔭 **Current Research** — GraphRAG, RAG-based KGQA, hybrid retrievers
- 📚 **Research Areas** — Graph Neural Networks (GNN) · Large Language Models (LLM) · Retrieval-Augmented Generation (RAG)
- 🌱 **Recent Interests** — Multilingual RAG, LLM fine-tuning, reasoning-enhanced retrieval
- 🏢 **Affiliation** — KAERI, Post-Master's Researcher (2026.03 – present)

---

## 🏆 Recent Highlights

- 📄 **SCIE** · [*Hybrid Retrieval-Augmented Generation*](https://www.mdpi.com/2076-3417/16/5/2244) — **Applied Sciences (MDPI)**, 2026.02 · *First author*
- 📄 **SCIE** · *Sequence-aware Adaptive GCRN for Traffic Forecasting* — Knowledge-Based Systems, 2025.11
- 📄 *Zero-Shot Industrial Anomaly Segmentation with Image-Aware Prompt Generation* — **PAKDD 2025** · *Co-first author*
- 🏆 **2nd Place · KISTI President's Award** — ScienceON AI Challenge 2025
- 🏅 **4th Place Special Award** — CIKM 2025 AnalytiCup (Alibaba Intl.)

---

## 🔥 Featured Projects

- **[HybRAG](https://github.com/2noweyh/HybRAG)** — Hybrid Node + Path Retriever for Knowledge Graph QA *(published in Applied Sciences, SCIE)*
- **[MPR-CiteG](https://github.com/2noweyh/MPR-CiteG)** — API-based RAG combining a Multi-Portfolio Retriever and a Citation-Grounded Generator
- **[IAP-ZSAS](https://github.com/2noweyh/IAP-ZSAS)** — Image-Aware Prompting for Zero-Shot Anomaly Segmentation *(PAKDD 2025)*

---

## 🛠 Tech Stack

**Core**
![Python](https://img.shields.io/badge/Python-3776AB?style=flat&logo=python&logoColor=white)
![PyTorch](https://img.shields.io/badge/PyTorch-EE4C2C?style=flat&logo=pytorch&logoColor=white)
![CUDA](https://img.shields.io/badge/CUDA-76B900?style=flat&logo=nvidia&logoColor=white)

**Graph & RAG**
![DGL](https://img.shields.io/badge/DGL-0A5E9C?style=flat)
![PyG](https://img.shields.io/badge/PyG-3C2179?style=flat)
![LangChain](https://img.shields.io/badge/LangChain-1C3C3C?style=flat&logo=langchain&logoColor=white)
![Neo4j](https://img.shields.io/badge/Neo4j-008CC1?style=flat&logo=neo4j&logoColor=white)
![FAISS](https://img.shields.io/badge/FAISS-0467DF?style=flat&logo=meta&logoColor=white)

**LLM / NLP**
![HuggingFace](https://img.shields.io/badge/HuggingFace-F5A623?style=flat&logo=huggingface&logoColor=white)
![Transformers](https://img.shields.io/badge/Transformers-FF6F00?style=flat)
![OpenAI](https://img.shields.io/badge/OpenAI-412991?style=flat&logo=openai&logoColor=white)

---

## 📊 GitHub Stats

![Stats](https://github-readme-stats-fast.vercel.app/api?username=2noweyh&show_icons=true&theme=tokyonight)
![GitHub Streak](https://streak-stats.demolab.com?user=2noweyh&theme=tokyonight&hide_border=true)

---

![Profile Views](https://komarev.com/ghpvc/?username=2noweyh&color=blue)
